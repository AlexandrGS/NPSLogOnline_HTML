...
#Не плутати з RefreshHTMLPageSec
...
$PowershellMinVersion = 7

#В цей масів заноситься інфо о кожній VPN сесії
[array]$Script:OnlineVPNSessions = @{}

#Значеня полів протокола Radius
#Packet-Type
$AccessRequest = 1
$AccessAccept = 2
$AccessReject = 3
...
#Куди виводити сповіщення
$Script:isPrintScreenOn = $True #на єкран
$Script:isDebugOn = $True       #в файл OutLogFile

#Вивід сповіщень
function PrintDebug($DebugMsg){

    [string]$CurrentDate = Get-Date -Format "dd:MM:yyyy HH:mm:ss"
    [string]$Msg = $CurrentDate + " " + $DebugMsg

    if($Script:isPrintScreenOn){
        Write-Host $Msg
    }
...
function GetIPGeoLocation([string]$IP){
    ...
    if(($Script:isFirstIPGeoLocationTest) -and ($Global:isIPGeoLocationEnable)){
        ...
        PrintDebug "Testing connection for geolocation service http://ipwhois.app"
        ...
        if(($FirstTest.query.success -eq 1) -and ( -not $ErrorOnTestingGeoLocServer)){
            PrintDebug "Testing connection for geolocation service OK. Geolocation enable. Free testing about 10000 IP address  to month"
        } else {
            PrintDebug "Testing connection for geolocation service OK. Geolocation disabled."
    ...
    if($Global:isIPGeoLocationEnable){
        ...
        if(-not $isIPInArray){
            $XMLWebRequest = [xml](Invoke-RestMethod -method Get -Uri "https://ipwhois.app/xml/$IP" -Proxy $Proxy -ProxyUseDefaultCredentials)
...
#        Write-Host "Всего" $Script:OnlineVPNSessions.Count "сессий на " (Get-Date)
        ...
        PrintDebug "Saving to HTML file $OutHTMLFile"
...
function FromFolderToFullPath([string[]]$Folders){
    ...
    $Msg = "Reading logs from files:" + $FullPath
    PrintDebug $Msg
...
#Оновлюн поле Status в массиве з описом кожноі VPN-сессії, чи помічае сесію на видаленя
#Якщо останній запис в логе для ціеї сесії менш $MaxOnlineSec секунд, то сессія в стані $StatusOnline
#Якщо пройшло секунд між $MaxOnlineSec і $MaxTimeOutSec, то сессія в стані $StatusWarning
#Якщо з останього запису в логе пройшло бiльше $MaxTimeOutSec но менше MaxDeleteSec секунд, то сессія в стані $StatusError
#Якщо з останього запису в логе пройшло бiльше MaxDeleteSec то сесiю остаточно видалити
function UpdateStatusForAllVPNSessions(){
    ...
    ForEach($I in $Script:OnlineVPNSessions){
        ...
        if($Sec -ge $MaxDeleteSec){
            $I.SessionID = "" #Сесію на видаленя
        }else{
        if($Sec -ge $MaxTimeOutSec){
                $I.Status = $StatusError + " [" + $Sec + "]" #Сесія в стані помилки
            }else{
                if($Sec -ge $MaxOnlineSec){
                    $I.Status = $StatusWarning + " [" + $Sec + "]" #Сесія в стані попередженя
                }else{
                    $I.Status = $StatusOnline + " [" + $Sec + "]" #Сесія в нормальному стані
                }
            }
        }
    }
...
function DeleteSessionFromVPNSessionsArray($XMLOneLineLog){
    ...
    ForEach($OneVPNSession in $Script:OnlineVPNSessions){
        if($OneVPNSession.SessionID -eq $XMLOneLineLog.Event."Acct-Session-Id"."#text"){
            ...
            $Msg = "Deleting session " + $XMLOneLineLog.Event."Acct-Session-Id"."#text" + " user " + $XMLOneLineLog.Event."User-Name"."#text"
            PrintDebug $Msg
            $isDeleted = $True
            Break
        }
    ...
    if(-not $isDeleted){
        $Msg = "Session " + $XMLOneLineLog.Event."Acct-Session-Id"."#text" + " user " + $XMLOneLineLog.Event."User-Name"."#text" + " not found for deleting."
        PrintDebug $Msg
    }
...
#        Write-Host $OLL
    ...
    ForEach( $I in $Script:OnlineVPNSessions){
        if( $I.SessionID -eq $AcctSessionID ){
            PrintDebug "Updating session $AcctSessionID user $UserName"
...
#            $I.Company = (Get-ADUser -Identity $I.UserName -Properties Company).Company #???Надо ли при каждой записи в лог обновлять компанию акаунта или достаточно один раз при создании сессии
    ...
    if( -not $isVPNSessionInArray){
        PrintDebug "Found session $AcctSessionID user $UserName"
        ...
        [bool]$boolErrorOnGetADUser = $False
        try{
            $OVPNSD = Get-ADUser -Identity $OneVPNSessionDesc.UserName -Properties Company,Department,Title
        }
        catch{
            PrintDebug "ERROR помилка при отриманi з Active Directory даних акаунта"
            $boolErrorOnGetADUser = $True
        }
        if($boolErrorOnGetADUser){
            $OneVPNSessionDesc.Company = ""
            $OneVPNSessionDesc.DurationHMS = ""
            $OneVPNSessionDesc.UserExternalIPGeolocation = ""
        }else{
            $OneVPNSessionDesc.Company = $OVPNSD.Company 
            if( ($OVPNSD.Department -ne "") -and ($OVPNSD.Department -ne $Null) -and ($OVPNSD.Department -ne $OneVPNSessionDesc.Company) ){
                $OneVPNSessionDesc.Company += ", " + $OVPNSD.Department
            }
            if( ( $OVPNSD.Title -ne "" ) -and ( $OVPNSD.Title -ne $Null) ){
                $OneVPNSessionDesc.Company += ", " + $OVPNSD.Title
            }
...
#    $OneLineLog = $_
    ...
    try {
        ...
        $Msg = "ERROR: String not converting to XML" + $_
        PrintDebug $Msg
    }
...
PrintDebug "Analyzing DTS-logs NPS-Radius Windows Server. Show online users. Version 02.04.2026"
PrintDebug "https://github.com/AlexandrGS/NPSLogOnline_HTML"
PrintDebug "Showing all users to 5-6 minutes"
#Get-Content $LogFiles -Wait -Tail $CountFirstReadLines | ForEach-Object { HandleOneLineLog  $_ }

#GetSeveralFilesContent ($LogFilesist)) | ForEach-Object { HandleOneLineLog  $_ }

#$QQQ = $LogFiles.Split($DelimiterOfFilesList)
#GetSeveralFilesContent ( $QQQ ) | ForEach-Object { HandleOneLineLog  $_ }

if ($PSVersionTable.PSVersion.Major -ge $PowershellMinVersion){
    PrintDebug "PowerShell meets the minimum version requirements $PowershellMinVersion"
}else{
    PrintDebug "Powershell must minimum $PowershellMinVersion version"
    return 1
}

#ForEach-Object -ThrottleLimit $LogPath.Count -Parallel {
FromFolderToFullPath $LogPath | ForEach-Object -ThrottleLimit $LogPath.Count -Parallel {
    $Log= $_ ;
    while($True){
        try{
            if(Test-Path $Log){
                ...
                PrintDebug "Path $Log not exists"
            }
            sleep 30s
        }
        catch{
            PrintDebug "Error on multithread loop on path $Log"
        }
    }
} | HandleOneLineLog

#GetSeveralFilesContent (FromFolderToFullPath $LogPath) | ForEach-Object { HandleOneLineLog  $_ }
PrintDebug "Work is completed"
return 0
